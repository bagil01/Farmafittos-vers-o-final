...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($resultado->num_rows === 0) {
        header('Location: ../../admin/pages/gerenciador_noticias.php?erro=login_invalido');
        exit;
    }

    $admin = $resultado->fetch_assoc();
    if (!password_verify($senha, $admin['senha'])) {
        header('Location: ../../admin/pages/gerenciador_noticias.php?erro=senha_incorreta');
        exit;
    }
    ...
    if ($stmtDelete->execute()) {
        header('Location: ../../admin/pages/gerenciador_noticias.php?sucesso=excluida');
    } else {
        header('Location: ../../admin/pages/gerenciador_noticias.php?erro=bd');
    }
